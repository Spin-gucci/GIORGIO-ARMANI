...
export function MasterDashboard({ user, users, deposits, withdrawals, products, systemBanks, stats }: MasterDashboardProps) {
  ...
  const handleCreateBank = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/system-banks", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          bankName,
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-2xl font-bold mt-1">{stats.total_customers}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-2xl font-bold mt-1">{formatCurrency(stats.total_deposits)}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">Total Withdraw</span>
              </div>
              <p className="text-2xl font-bold mt-1">{formatCurrency(stats.total_withdrawals)}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4">
              ...
              <p className="text-2xl font-bold mt-1">{parseInt(stats.pending_deposits) + parseInt(stats.pending_withdrawals)}</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="users" className="space-y-4">
          <TabsList className="flex flex-wrap">
            ...
            <TabsTrigger value="deposits"><ArrowDownCircle className="h-4 w-4 mr-2" />Deposits</TabsTrigger>
            <TabsTrigger value="withdrawals"><ArrowUpCircle className="h-4 w-4 mr-2" />Withdrawals</TabsTrigger>
            <TabsTrigger value="products"><Package className="h-4 w-4 mr-2" />Products</TabsTrigger>
            <TabsTrigger value="banks"><Building2 className="h-4 w-4 mr-2" />Banks</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="banks">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <CardTitle>System Banks</CardTitle>
                <Dialog open={bankOpen} onOpenChange={setBankOpen}>
                  <DialogTrigger asChild>
                    <Button size="sm"><Plus className="h-4 w-4 mr-2" />Add Bank</Button>
                  </DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Add System Bank</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleCreateBank} className="space-y-4">
                      <div className="space-y-2">
                        <Label>Bank Name</Label>
                        <Input value={bankName} onChange={(e) => setBankName(e.target.value)} placeholder="BCA, Mandiri, etc" required />
                      </div>
                      <div className="space-y-2">
